...
contract Oracle {
    ...
    mapping(uint256 => mapping(bytes32 => uint256))
        private rewardsMerkleRootvotes;
    ...
    uint256 public activeBlockNumber;
    ...
    event RewardsMerkleRootConfirmed(
        uint256 blockNumber,
        bytes32 winningRootValue
    );

    /**
     * Gas Reimbursement Variables
     **/
    // Record the
    mapping(address => uint256) gasReimbursementClaimableBalance;
    ...
    function submitRewardsMerkleRoot(
        uint256 blockNumber,
        bytes32 submittedRootValue
    ) external {}

    // called when > 51% votes is gathered
    function confirmRewardMerkleRoot(
        uint256 blockNumber,
        bytes32 winningRootValue
    ) internal {}
     ...
     **/
    // Q: For correct oracle member who submit the correct merkleroot value, should they be reimbursed?
    // called by oracle member wallet, transfer FIL from this contract to oracle member wallet
    function claimGasReimbursement(uint256 amount) external {}

    /**
     * Start LoanAgent Functions
     **/
